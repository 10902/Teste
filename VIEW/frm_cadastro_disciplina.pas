...
begin
  If Application.MessageBox('Cancelar cadastro ?', 'Atenção !!!',
...
begin
  ...
  begin
    If Application.MessageBox('Confirmar cadastro ?', 'Atenção !!!',
      MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) = IDYES Then
    begin
      ...
      begin
        ShowMessage('Nome da disciplina é obrigatório !');
      end;
    end;
  end;
  ...
  begin
    If Application.MessageBox('Confirmar alteração ?', 'Atenção !!!',
      MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) = IDYES Then
    begin
      if Length(Trim(edit_descricao.Text)) > 0 then
      begin
