      ...
      begin
        ...
        begin
          ...
          begin
            ...
            l := lista.Items.Add;
            l.Caption := IntToStr(DT.Sql_Aluno.fieldByName('disciplina').Value);
  ...
  TRY
    ...
    try
      begin
        ...
        DT.Sql_Aluno.SQL.Add
          ('select descricao from v_aluno_disciplina where aluno=:aluno and disciplina =:disciplina and status=1');
        DT.Sql_Aluno.ParamByName('aluno').AsInteger := p.id;
        DT.Sql_Aluno.ParamByName('disciplina').AsInteger := disciplina;
        DT.Sql_Aluno.open;
        if not(DT.Sql_Aluno.IsEmpty) then
        begin
          ShowMessage('Essa disciplina já está vinculada para este aluno !');
        end
        else
        begin // se não estiver vinculada ainda, vamos vincular
          ...
          DT.Sql_Aluno.ParamByName('dt_cadastro').AsDateTime := p.datacadastro;
          DT.Sql_Aluno.ParamByName('dt_atualizacao').AsDateTime :=
            p.dataatualizacao;
          DT.Sql_Aluno.ParamByName('user_atualizacao').AsInteger :=
            p.useratualizacao;
