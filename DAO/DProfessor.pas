  ...
  TRY
    ...
    try
      begin
        ...
        DT.Sql_professor.SQL.Add
          ('update tb_professor_disciplina set status=:status,dt_atualizacao=:dt_atualizacao,user_atualizacao=:user_atualizacao where professor=:professor and disciplina=:disciplina');
...
procedure TDprofessor.ListarDisciplinas(professor: integer; lista: TListView);
var
  DT: TDM;
  l: TListItem;
  ...
  TRY
    ...
    try
      begin
        ...
        DT.Sql_professor.SQL.Add
          ('select disciplina,descricao from v_professor_disciplina where professor=:professor and status=1');
        DT.Sql_professor.ParamByName('professor').AsInteger := professor;
        DT.Sql_professor.open;
        lista.Items.Clear;
        if not(DT.Sql_professor.IsEmpty) then
        begin
  ...
  TRY
    ...
    try
      begin
        ...
        begin
          ShowMessage
            ('Essa disciplina já está vinculada para este professor !');
        end
        else
        begin // se não estiver vinculada ainda, vamos vincular
          ...
          DT.Sql_professor.SQL.Add
            ('insert into tb_professor_disciplina (professor,disciplina,dt_cadastro,dt_atualizacao,user_atualizacao) ');
          DT.Sql_professor.SQL.Add
            ('values (:professor,:disciplina,:dt_cadastro,:dt_atualizacao,:user_atualizacao)');
          DT.Sql_professor.ParamByName('professor').AsInteger := p.id;
          DT.Sql_professor.ParamByName('disciplina').AsInteger := disciplina;
